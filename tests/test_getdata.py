...
async def test_fetch_get_returns_binary(mock_aioresponses, api_instance):
    api = api_instance
    pattern = re.compile(getwowdata.urls['item_icon'].format(region=api.region, item_id=1))
    data = {'key':1}
    mock_aioresponses.get(pattern, status=200, payload=data, headers={'Date':'Tue, 02 Aug 2022 04:05:31 GMT'})
...
async def test_fetch_search_items(mock_aioresponses, api_instance):
    api = api_instance
    search_pattern = re.compile(getwowdata.urls['search_item'].format(region=api.region))
    ...
    mock_aioresponses.get(search_pattern, status=200, payload=search_data, headers={'Date':'Tue, 02 Aug 2022 04:05:31 GMT'})
    mock_aioresponses.get(pattern_dummyurl, status=200, payload=item_data, headers={'Date':'Tue, 02 Aug 2022 04:05:31 GMT'})
